# Kronagent Testbed — local cloud-alert-stream simulation

Runs the **live ingestion path** (GuardDuty → EventBridge → SQS, long-polled by
`kronagent.ingestion.SqsFindingSource`) end-to-end with **no AWS account, no
...
Kronagent ingests **only SQS**, and the stack is pure-Python with a "runs anywhere,
no heavy deps" ethos (lazy `boto3`, graceful degradation). So:

| Option | Setup | Auth | Fidelity | Verdict for Kronagent |
|---|---|---|---|---|
...
`Settings.sqs_endpoint_url` (env `KRONAGENT_SQS_ENDPOINT_URL`) overrides the SQS
...
This script will:
1. Spin up a Kind cluster named `kronagent-testbed` (disabling default `kindnet`).
