...
"""
Kronagent testbed — local SQS emulator + async finding stream.
...
ingestion path (kronagent.ingestion.SqsFindingSource, boto3 long-polling) run
...
Why moto server and not LocalStack:
  ...
  (matches the Kronagent stack), and exposes a real SQS endpoint — including the
...
Usage:
  ...
  KRONAGENT_SQS_ENDPOINT_URL=... plus KRONAGENT_SQS_QUEUE_URL=...  (printed on serve) -> run_slice.py ingests live
"""
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    print("\n=== Kronagent SQS testbed ready ===", flush=True)
    ...
    print(f"  export KRONAGENT_SQS_ENDPOINT_URL={endpoint}", flush=True)
    print(f"  export KRONAGENT_SQS_QUEUE_URL={queue_url}", flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Kronagent local SQS testbed")
    sub = parser.add_subparsers(dest="command", required=True)

    p_serve = sub.add_parser("serve", help="start the SQS emulator and stream findings into it")
    p_serve.add_argument("--port", type=int, default=int(os.getenv("KRONAGENT_TESTBED_PORT", "5001")))
    ...
    p_enq.add_argument("--endpoint-url", default=os.getenv("KRONAGENT_SQS_ENDPOINT_URL", ""),
                       help="SQS endpoint (e.g. http://localhost:9324 for ElasticMQ)")
    p_enq.add_argument("--queue-url", default=os.getenv("KRONAGENT_SQS_QUEUE_URL", ""),
                       help="queue URL; if omitted, a queue named 'kronagent-findings' is created")
